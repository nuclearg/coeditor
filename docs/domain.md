...
> **本文档是领域模型的唯一权威来源。**
> 任何实现（客户端 / 服务端）若与本文冲突，以本文为准。
> 修改领域模型必须先改本文，再同步相关实现与 `docs/api.md`。
...
- 类型定义：`packages/shared/src/types.ts` 为权威类型，客户端与服务端共享。
- 会话归属 draftId：会话创建支持 `draftId`（契约见 `docs/api.md`），消息按版本桶归属。
- 会话分桶：前端 `conversationStore`/`AiPanel` 按 `draftId` 分桶（`bucketId = draftId || parentId`）。
- 按 draft 读内容：`lib/prompt-context.ts`（`draftContent`）。
- 审阅新开会话：`AiPanel.autoSubmit` 总是 `createConversation`。

## 6. 变更纪律

1. 修改领域模型 → 先改本文。
2. 实现与 `docs/api.md` 必须同步，不允许长期偏离。
