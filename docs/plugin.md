...
- 默认自带 `settings`、`user` 插件；部署方可替换注册表注入自己的插件集
...
`pages/settings` 是一个**内置普通页面**：固定形态 `LayoutShell variant="settings"`（与 home/editor 齐名），壳与首页（index）一致——main.head 显示 logo + 页面标题，内容全宽滚动，页脚版权。内容区开放 **`settings.body` 插槽**（链式装饰机制，与其它 slot 一致；整页式自定义内容可直接忽略 defaults 返回自身布局）：

- **默认注册表**：`settingsPlugin` → 偏好区（审阅风格/CoT/主题/语言）+ API 配置（BYOK）+ 数据目录
- **部署方自定义**：可通过自定义注册表注入自己的插件替换 `settings.body`（例如自定义的偏好/资料页），同样注册 `settings.body`

```ts
// 插件注册设置页内容（ui.slots）+ 页面标题/齿轮底部入口文案（缺省"高级设置"）
export const hostPlugin: CoEditorPlugin = {
  id: 'host',
  i18n: {
    zh: { settingsTitle: '我的设置' },
    en: { settingsTitle: 'My Settings' },
  },
  settingsPageLabel: () => t('plugin.host.settingsTitle'),  // 页面标题 + 齿轮底部入口文案
  ui: {
    slots: {
      'settings.body': mySettingsBodySlot,   // 设置页内容（SlotRenderer；可忽略 defaults）
    },
  },
}
```
...
框架预留 10 个静态自定义页面（`pages/custom/{1..10}`，app.config 静态声明），插件通过 `pages.custom[N]` 注册组件填充（缺省显示占位提示）。适合**独立页面形态**的功能（如向导/帮助等整页内容），避免塞进 slot 破坏布局。
...
interface CoEditorPlugin {
   ...
   * 预留 10 个静态扩展页面，插件注册组件填充（缺省显示占位提示）。
   * 适合独立页面形态的功能（如向导/帮助等整页内容），避免塞进 slot 破坏布局。
   */
  pages?: {
   ...
   * 缺省为内置文案（"高级设置"/"Advanced Settings"）；插件可自定义。
...
export const hostPlugin: CoEditorPlugin = {
  id: 'host',
  i18n: {
    zh: { card: '用户卡片', guide: '使用指南', home: '项目主页' },
    en: { card: 'User Card', guide: 'Guide', home: 'Project Home' },
  },
  ui: {
    slots: {
      // 设置入口：👤 人头替换齿轮
      ...
      // 菜单末尾：用户卡片
      'settings-menu': () => (
        <View className="menu-label">{t('plugin.host.card')}</View>
      ),
    },
  },
  settings: {
    menuItems: [
      { type: 'link', label: () => t('plugin.host.home'), url: 'https://example.com' },
      { type: 'action', label: () => t('plugin.host.guide'), onClick: () => Taro.navigateTo({ url: '/pages/custom/1' }) },
    ],
  },
}
```
...
| `pages.custom[N]` | **自定义页面位**（`pages/custom/{1..10}`）：插件注册组件填充独立页面（缺省占位提示）。适合向导/帮助等整页形态，页面用 `LayoutShell variant="custom"` 渲染全宽内容 |
| `settings.body` | **设置页内容**（`pages/settings`，布局与首页一致）：注册到 `ui.slots` 的链式插槽，填充/装饰设置页内容区。默认=偏好+BYOK+dataDir；部署方可替换为自定义内容 |
| `settingsPageLabel` | **设置页标题 + 齿轮底部入口文案**：插件自定义，缺省"高级设置"。惰性函数支持语言切换；设置页 head 面包屑与 H5/小程序标题由页面读取该值 |
...
内置用户插件（示例实现），`user.get` 直接返回固定用户名（`default-user`），纯前端实现、不请求后端。
...
### 示例：注册设置页内容（settings.body）

```tsx
// 插件把设置页内容注册到 settings.body 插槽（页面壳由框架提供，与首页布局一致）
import type { CoEditorPlugin } from '@/plugin'
...
export const hostPlugin: CoEditorPlugin = {
  ...
  i18n: {
    zh: { pageTitle: '我的设置' },
    en: { pageTitle: 'My Settings' },
  },
  settingsPageLabel: () => t('plugin.host.pageTitle'),   // 页面标题 + 齿轮入口文案
  ui: {
    slots: {
      'settings.body': () => <MySettingsPage />,         // 整页式内容：忽略 defaults 返回自身布局
    },
  },
}
```
...
```tsx
// 插件注册 page.custom.1（自定义页面），设置菜单入口 navigateTo 进入
import type { CoEditorPlugin } from '@/plugin'
...
export const hostPlugin: CoEditorPlugin = {
  ...
  settings: {
    menuItems: [
      { type: 'action', label: () => t('plugin.host.open'), onClick: () => Taro.navigateTo({ url: '/pages/custom/1' }) },
    ],
  },
}
```
