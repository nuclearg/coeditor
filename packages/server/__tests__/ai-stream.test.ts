...
describe('CoT 开关 → 上游 thinking 参数', () => {
  it('showThinking=false 时请求体带 thinking:disabled；默认/开启时不携带（兼容 BYOK 上游）', async () => {
    ...
    // 默认（CoT 开，DeepSeek 默认开启思考）：请求体不带 thinking
    const turnId = await createTurn('cot默认')
    await chatRequest(turnId)
    expect(bodies[0].thinking).toBeUndefined()

    // 关闭 CoT：请求模型直接不生成思维链（省 token），而非生成后再丢弃
    await rpcOk('settings.update', { showThinking: false })
    const turnId2 = await createTurn('cot关闭')
    await chatRequest(turnId2)
    expect(bodies[1].thinking).toEqual({ type: 'disabled' })

    // 恢复默认，避免影响后续用例
    await rpcOk('settings.update', { showThinking: true })
  })
})
