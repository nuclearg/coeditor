...
export function SettingsMenu({ showReviewStyle = false }: SettingsMenuProps) {
  ...
  const goSettings = () => {
    setOpen(false)
    // 插件注册了设置启动器（桌面弹窗）则用之，否则走路由设置页（Web/小程序）
  ...
  const handleGearClick = () => {
    // 插件注册了设置启动器（桌面齿轮直开弹窗）；否则保持轻量下拉（Web/小程序）
    const launcher = getSettingsLauncher()
    if (launcher) {
      launcher()
      return
      ...
      {open && (
        <>
          ...
          <View className="menu-panel">
            ...
            {/* 编辑页专属：CoT 思考开关（部署方可将 cotSelectable 置 false 隐藏） */}
            ...
            {/* 插件扩展点（menuItems + settings-menu 插槽）：无内容时整个省略，
                避免空区两侧残留分隔线（主题 ↔ 插件扩展菜单之间出现多余横线的根因） */}
            ...
            {/* 设置页入口：固定放菜单最底（文案与设置页标题共用，见 getSettingsPageLabel） */}
            <View className="menu-sep" />
