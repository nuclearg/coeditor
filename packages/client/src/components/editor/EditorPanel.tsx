  ...
  return (
    <View className="flex flex-col" style={isMobile ? undefined : { flex: 1, minHeight: 0 }}>
      ...
      {/* editorpanel.body：书写区（无容器 padding；textarea 内部自带留白，顶部贴紧 tabs）。
         桌面端容器只负责布局、不做滚动（overflow hidden），滚动统一交给唯一的内层：
         可编辑=textarea 内部滚动；只读（全文/章节）= 下方 View 内部滚动，避免双滚动条。 */}
      <SlotHost
              ...
              isEditable ? (
                <Textarea
                  ...
                  style={isMobile
                    ...
                    : { height: '100%', minHeight: 0, boxSizing: 'border-box', border: '1px solid var(--border)' }}
                  disabled={contentLoading}
                />
              ) : (
                // 只读视图（全文/章节）：无圆角，高度撑满 body，内容多时由本 View 自身滚动（唯一滚动容器）
                <View style={{ height: '100%', minHeight: 0, boxSizing: 'border-box', overflowY: 'auto', padding: isWebView() ? 12 : 16, border: '1px solid var(--border)' }}>
