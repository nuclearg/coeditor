...
export function useDraftManager({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        // 串行执行：并发 ensure 会对同一文档产生 @Version 乐观锁冲突（StaleObjectState → 500）
  ...
  useEffect(() => {
    ...
    // 切到新目标时：若本地有未保存快照（误刷新/重进文档前的编辑），先与服务端
    // 数据比时间——服务端更新则丢弃过期快照、用服务端内容（多端同步：防止某台
    // 设备残留的旧快照覆盖另一台设备上的新保存）；本地快照更新则恢复它并标记未保存。
    if (selectionChanged && selectionKey && docId) {
      const stored = getDraftSnapshot(docId, selectionKey)
      if (stored) {
        // 服务端当前版本创建时间 ≈ 该目标最近一次保存时间
        const serverCreatedAt = editingAttachmentId ? currentAttachmentDraft?.timeCreated : currentDraft?.timeCreated
        const serverTs = serverCreatedAt ? new Date(serverCreatedAt).getTime() : 0
        if (serverTs > stored.updatedAt) {
          // 服务端更新：用服务端数据替换本地 localStorage（清除过期残留快照）
          clearDraftSnapshot(docId, selectionKey)
        } else if (stored.content !== serverContent) {
          // 本地快照更新且内容不同：恢复本地未保存内容
  ...
  const attachmentsText = useMemo(() => {
    ...
    for (const def of template.attachments) {
      ...
      const label = def.contextLabel?.zh ?? def.contextLabel?.en ?? def.type
      parts.push(`[${label}]\n${cur.content}`)
    }
    return parts.join('\n\n')
  ...
  // 批量删除（tab 右键菜单：关闭右侧/关闭其它）。逐版本走与单个删除相同的
  // 附件/段落分支；删除集包含当前编辑版本时清 dirty。
  const handleDraftsDelete = useCallback(async (draftIds: string[]) => {
    if (!docId || draftIds.length === 0) return
    for (const draftId of draftIds) {
      if (editingAttachmentId) await deleteAttachmentDraft(docId, editingAttachmentId, draftId)
      else if (selection) await deleteDraft(docId, selection.chapterId, selection.paragraphId, draftId)
    }
    const includesCurrent = editingAttachmentId
      ? draftIds.includes(currentAttachmentDraft?.id || '')
      : draftIds.includes(currentDraft?.id || '')
    if (includesCurrent) {
      setDirty(false)
      dirtyRef.current = false
    }
