// English locale
export default {
  // Common
  'common.save': 'Save',
  'common.rename': 'Rename',
  'common.renameFailed': 'Rename failed',
  'common.cancel': 'Cancel',
  'common.delete': 'Delete',
  'common.export': 'Export',
  'common.confirm': 'Confirm',
  'common.loading': 'Loading...',
  'common.close': 'Close',
  'common.retry': 'Retry',
  'brand.name': 'CoEditor',
  'footer.copyright': '© 2026 CoEditor',
  'common.untitled': 'Untitled',
  'common.empty': '(empty)',
  'common.on': 'On',
  'common.off': 'Off',
  'common.enable': 'Enable',
  'common.disable': 'Disable',

  // Document list
  'doc.newTitle': 'New document title...',
  'doc.create': 'Create',
  'doc.deleteTitle': 'Delete document',
  'doc.deleteConfirm': 'Delete "{title}"? This cannot be undone. All chapters and paragraphs will be removed.',
  'doc.deleting': 'Deleting...',
  'doc.emptyHint': 'No documents yet. Create one to get started.',

  // Sidebar
  'sidebar.fulltext': 'Full Text',
  'sidebar.chapters': 'Chapters',
  'sidebar.paragraphN': 'Paragraph {n}',
  'sidebar.newParagraph': 'New paragraph...',
  'sidebar.newChapter': 'New chapter...',
  'sidebar.moveUp': 'Move up',
  'sidebar.moveDown': 'Move down',
  'sidebar.rename': 'Rename',
  'sidebar.wordCount': '{n} ch',
  'sidebar.deleteChapter': 'Delete',
  'sidebar.deleteChapterConfirm': 'Delete chapter "{name}" and all its paragraphs? This cannot be undone.',
  'sidebar.deleteParagraphConfirm': 'Delete "{name}"? This cannot be undone.',
  'sidebar.fulltextCount': '{n} ch',

  // Editor
  'editor.writingArea': 'Writing Area',
  'editor.startWriting': 'Start writing...',
  'editor.writeAttachment': 'Edit {name}...',
  'editor.unsaved': 'Unsaved',
  'editor.saving': 'Saving...',
  'editor.review': 'Review',
  'editor.reviewAndSave': 'Review',
  'editor.export': 'Export',
  'editor.unsavedTitle': 'Unsaved changes',
  'editor.unsavedHint': 'You have unsaved changes. Save before switching?',
  'editor.discard': 'Discard',
  'editor.saveAndSwitch': 'Save & switch',
  'editor.draftRestored': 'Restored unsaved draft — remember to save',

  // AI panel
  'ai.fulltextReview': 'Full-text Review',
  'ai.attachmentReview': 'Attachment Review',
  'ai.chapterReview': 'Chapter Review',
  'ai.paragraphReview': 'Paragraph Review',
  'ai.assistant': 'AI Assistant',
  'ai.reviewOpinion': 'Opinion {n}',
  'ai.submittedFulltext': 'Submitted full-text review',
  'ai.submittedAttachment': 'Submitted attachment review',
  'ai.submittedChapter': 'Submitted chapter review',
  'ai.submittedParagraph': 'Submitted paragraph review',
  'ai.question': 'Question',
  'ai.requestFailed': 'Request failed',
  'ai.thinking': 'Thinking',
  'ai.thinkingInProgress': 'Thinking...',
  'ai.reviewPlaceholder': 'Ask for editing advice...',
  'ai.questionPlaceholder': 'Ask the AI...',
  'ai.reviewRequest': 'Please review and give suggestions.',
  'ai.disclaimer': 'This answer is provided by the AI editor and for reference only. Please verify carefully.',

  // Draft versions
  'drafts.noHistory': 'No version history',
  'drafts.deleteTitle': 'Delete version',
  'drafts.deleteConfirm': 'Delete this version? This cannot be undone.',
  'drafts.justNow': 'just now',
  'drafts.minutesAgo': '{n}m ago',
  'drafts.hoursAgo': '{n}h ago',

  // Tab context menu
  'tabMenu.closeRight': 'Close tabs to the right',
  'tabMenu.closeOthers': 'Close others',
  'tabMenu.closeRightConfirm': 'Close {count} {noun} to the right? This cannot be undone.',
  'tabMenu.closeOthersConfirm': 'Close the other {count} {noun}? This cannot be undone.',
  'tabMenu.nounConversation': 'conversations',
  'tabMenu.nounDraft': 'versions',

  // Settings menu
  'settings.reviewStyle': 'Review Style',
  'settings.gentle': 'Gentle',
  'settings.strict': 'Strict',
  'settings.praise': 'Encouraging',
  'settings.theme': 'Theme',
  'settings.appearance': 'Appearance & Language',
  'settings.light': 'Light',
  'settings.dark': 'Dark',
  'settings.apiConfig': 'API Config',
  'settings.advanced': 'Advanced Settings',
  'settings.showThinking': 'Show thinking (CoT)',
  'settings.dataDir': 'Data Directory',
  'settings.dataDirHint': 'All data (documents, chapters, drafts, AI conversations) is stored in this directory. The switch takes effect after a restart; existing data is not moved.',
  'settings.dataDirCurrent': 'Current directory',
  'settings.dataDirNew': 'New directory path',
  'settings.dataDirPick': 'Browse…',
  'settings.dataDirSave': 'Switch & save',
  'settings.dataDirSaved': 'Switched. The new directory is now in effect.',
  'settings.dataDirEmpty': 'Please enter a directory path',
  'settings.dataDirPersistHint': 'Takes effect after restart',

  // Home page
  'home.count': '{n} works',
  'home.createWorks': 'Create Work',
  'home.import': 'Import',
  'home.importText': 'Import Text',
  'home.importFile': 'Import File',
  'home.importing': 'AI analyzing…',
  'home.importTooLarge': 'Content too long (max 100,000 chars). Please split it first',
  'home.importFailed': 'Import failed',
  'home.exportFailed': 'Export failed',
  'home.importTextPlaceholder': 'Paste your text here. AI will split it into chapters and break each chapter into ~1,000-character paragraphs (max 100,000 chars)',
  'home.importPrereq': 'Select a template and enter a title before importing',
  'home.allWorks': 'All Works',
  'home.templatePlaceholder': 'Choose a template…',
  'home.timeUpdated': 'Updated {date}',

  // API config
  'apiConfig.title': 'AI API Config',
  'apiConfig.provider': 'Model provider',
  'apiConfig.providerCustom': 'Custom (OpenAI-compatible)',
  'apiConfig.providerZenMisc': 'Other models (opencode zen)',
  'apiConfig.chooseModel': 'Choose a model…',
  'apiConfig.model': 'Model',
  'apiConfig.modelHint': 'The list comes from the opencode zen model catalog; pick “Custom (OpenAI-compatible)” to type any model ID',
  'apiConfig.modelManualHint': 'Any OpenAI-compatible model ID works (e.g. a local ollama model name)',
  'apiConfig.apiKey': 'API Key',
  'apiConfig.apiBaseUrl': 'API Base URL',
  'apiConfig.baseUrlHint': 'When switching providers, the gateway default is auto-filled unless you edited it manually',
  'apiConfig.modelsLoading': 'Loading model catalog…',
  'apiConfig.modelsLoadFailed': 'Failed to load the model catalog; you can type a model ID manually',
  'apiConfig.save': 'Save settings',
  'apiConfig.saved': 'Saved',
  'apiConfig.keyHint': 'The key is stored on your local server and never uploaded to third parties.',

  // Network / stream errors
  'error.server': 'Unexpected server response',
  'error.requestFailed': 'Request failed',
  'error.readStream': 'Failed to read response stream',
  'error.network': 'Network request failed',
  'error.aiRequest': 'Network request failed. Check your API Key and API URL.',
  'error.saveFailed': 'Save failed, please retry',
  'error.loadFailed': 'Loading failed, please retry',
  'error.operationFailed': 'Operation failed',
  'error.crashed': 'Something went wrong',
  'error.reload': 'Reload',
}
