...
export default function DocumentListPage() {
  ...
  // 登录门场景（部署方注入登录态时）：未登录进入首页时首次加载被 401 拦截（PluginHandled 静默），
  ...
  /** 导入前置条件：必须先选择模板 + 填写标题（与创建作品共用同一组输入） */
  const canImport = Boolean(selectedTemplate) && title.trim().length > 0
  ...
  /** 提交导入（文件/文字共用）：服务端 AI 分章 → 跳转新文档。标题与模板必填（调用方已校验） */
  const submitImport = async (docTitle: string, docTemplateId: string, content: string) => {
    setImporting(true)
    try {
  ...
  /**
   * 导入文件（H5/桌面端）：前置校验（已选模板 + 已填标题）→ 文件选择 → 读文本 → AI 分章 → 跳转新文档。
            ...
            <View className="flex gap-2 mt-2" style={{ flexWrap: 'wrap' }}>
              ...
              {isH5() && (
                <>
                  ...
                  <Button variant="outline" onClick={handleImport} disabled={importing || !canImport}>
                    {t('home.importFile')}
                  </Button>
                </>
              )}
      ...
      >
        ...
        <View className="flex justify-end gap-2" style={{ marginTop: 18 }}>
          ...
          <Button
            ...
            disabled={importing || !textContent.trim() || !canImport}
          >
