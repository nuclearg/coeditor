...
- 构建：`build:h5`（产物 dist-h5）。**开源版不产出小程序包**：`build:weapp` / `dev:weapp` 脚本与 `project.config.json`（含微信 appid）均不在本仓库。微信小程序由 SaaS 宿主（coeditor-saas，以 git submodule 引用本仓库源码）自行驱动 `taro build --type weapp`，appid 由 SaaS 侧提供；源码中的双端分支与 `@tarojs/plugin-platform-weapp` 依赖即为此保留（Taro 本身不读 `project.config.json`，该文件仅供微信开发者工具使用）。**小程序特有的构建配置同样不在基座**：`mini` 块只留平台中立的 `webpackChain`（把 shared/宿主插件纳入 babel），产物目录与 postcss、以及 `theme.json`（darkmode 变量表）由宿主提供，经 `config/index.ts` 末尾的平台中性注入点 `TARO_CONFIG_EXTEND`（模块绝对路径，深合并）+ 构建后拷贝落地——Taro 4.2 没有 `--config` 这类"换配置文件"开关，故留这个缝（与 `PLUGIN_REGISTRY_PATH`/`PLUGIN_EXTRA_INCLUDE`/`API_BASE_URL` 同一套思路）
