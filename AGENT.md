...
- 构建：`build:h5`（产物 dist-h5）。**开源版只构建 H5，不含任何小程序构建配置/资产/依赖**：`build:weapp` / `dev:weapp` 脚本、`project.config.json`（含微信 appid）、`theme.json`（darkmode 变量表）、`mini` 的产物目录与 postcss、`app.json` 的小程序专属键（window 的 `@变量`、`themeLocation`、`darkmode`）、以及 `@tarojs/plugin-platform-weapp` 依赖，**都不在本仓库**。小程序由 SaaS 宿主（coeditor-saas，以 git submodule 引用本仓库源码）驱动 `taro build --type weapp`，细节见其 AGENT.md：微信工程/appid、构建块（经基座 `config/index.ts` 末尾的平台中性注入点 `TARO_CONFIG_EXTEND` 深合并——Taro 4.2 没有 `--config` 这类换配置文件开关，故留这个缝）、构建后 patch `dist-weapp/app.json` 与放入变量表、平台插件（宿主的依赖树里有，Node 从 client 目录向上解析，已验证）。基座保留的只是**双端运行时分支**（`isWebView()` / `process.env.TARO_ENV` 等）：同一份源码要能被编译成小程序，这与"基座不构建小程序"并不矛盾
...
- **设置下拉菜单**（`components/settings/SettingsMenu.tsx`）：审阅风格 radio（核心，非插件，仅编辑页显示 `showReviewStyle`）+ 插件菜单项；数据目录设置由 **data-dir 插件**提供（默认在 `plugin/registry.ts` 注册，部署方可不注册即裁剪掉该入口）
